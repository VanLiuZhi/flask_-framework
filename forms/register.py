...
from flask_security.forms import (
    ...
    ValidationError, SendConfirmationForm, get_message,
    email_required, email_validator, get_form_field_label, valid_user_email,
    PasswordField, password_required, verify_and_update_password, EqualTo)
...
class ExtendedLoginForm(LoginForm):
    email = StringField('邮箱/用户名',
                        validators=[Required(message='未输入账号内容')])
    password = PasswordField('密码',
                             validators=[password_required])


class ExtendedRegisterForm(RegisterForm):
    name = StringField('名字', validators=[name_required, name_length,
                                         unique_user_name])
